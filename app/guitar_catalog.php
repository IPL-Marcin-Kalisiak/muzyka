...
function guitarScores(string $directory): array
{
    $scores = [];
    foreach (glob($directory . '/*.json') ?: [] as $path) {
        ...
        foreach ($score['tracks'] as $track) {
            if (!is_array($track) || !is_string($track['instrument'] ?? null) || preg_match('/(^|_)guitar($|_)/i', $track['instrument']) !== 1) continue;
            $count += is_array($track['notes'] ?? null) ? count($track['notes']) : 0;
            foreach ($track['voices'] ?? [] as $voice) {
                $count += is_array($voice['notes'] ?? null) ? count($voice['notes']) : 0;
            }
        }
        if ($count === 0) continue;
